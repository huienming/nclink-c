...
> **报警那一列在采样里可能是 `null`**：采样编码里"某项的值是空表"和"本周期该项没有
> 数据"的占位同形，设备端发布前会把这种列换成 `null`。所以 `/CONTROLLER/WARNING`
> 在采样报文里是 `null`（没有报警）或 `[{"number","text"}]`（有报警）—— 要区分
> "没有报警"和"读不到"，用 Query 按路径读它。
