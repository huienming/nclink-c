...
#### 11.22.1 ⚠️ 本 client 的写**还没落地**（下一轮第一件事）

同一台机器上跑 `focas_pmc_rw.exe scan` 扫了一圈（K 0..7、R 1000..1007、R 5000..5007、
D 100..107）：**本 client 写出去的值读回来都没到位**（K/R 全 0、D 只有零星几个字对得上），
而**官方库同一个地址写就能落**（上面那条 0x32/0x44 就是官方库写的）。两边帧形状看着
一模一样（`0x8002` + 同一个四格载荷 + `[长度 BE32][数据]`、块头第 2 格都是 2）——
**所以还差一轮"把官方库那次能落的帧与本 client 的帧逐字节 diff"**（工具都在：
`focas_sdk_probe` + `cap.py` 抓官方库、`focas_pmc_rw.exe` 抓本 client）。

**在那之前，页面上这一格按"读通了、写还没通"看**：适配器已经声明了 `set_value`
（`@Y @G @R @K @D`），但按现在的实现写了不落地 —— 要么修到落地，要么把 `set_value`
收回去，不留一个"回 rc=0 却没写进去"的口子。

> 用户提示的下一步：**去 VM 的 PMC 画面 / SERVO GUIDE 的 I/O 监视**挑一个梯形图不碰的
> 号（比如 `D` 的空白号、R 系统区之外的号）复核 —— 位置、值两边一起看，能同时排掉
> "梯形图改了值"和"写根本没发生"这两种可能。
