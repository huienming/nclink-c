...
实测（同一台 NCGuide 0i-MF，**官方库**写的，再用本 client 读回来）：

```
官方库：写 R0..R1 = 0x33 0x44（s=0 e=1 length=10）→ rc=0
本 client 读回 R 0..7 = [0x32, 0x44, 0, …]     ← 值确实落地了
```
...
* **有的号写不进去**：读回 R0 是 `0x32` 而写的是 `0x33` —— R0 那一位被**跑着的 PMC
  梯形图**当场改回去了。这就是用户说的"有的寄存器不能写"：能写的族也不保证每一位都听
  你的（X/F 是机床与 CNC 驱动的信号，本来就只读）。

#### 11.22.1 写落地了：错在**载荷多塞了 4 个字节**（2026-09-23 啃掉）

症状：帧形状看着与官方库一样，机床回 `rc=0`，可值就是不到位（K/R/D 扫下来全不落），
而**官方库写同一个地址能落**（写 `R0..R1 = 0x33 0x44` → 本 client 读回 `[0x32, 0x44]`）。

逐字段比两边的帧才看出来：官方库**写**那帧的块尾是

```
... [s=0][e=1][族][宽度] | 0000 0002 | AA 55
                          ^^^^^^^^^   ^^^^^
                          长度 4 字节   数据
```

—— **长度那一格正好压在块头末尾那两格上**（`[24..28)` 读成 BE32 就是"要写几个字节"），
我们早先把长度**又**塞进载荷里，等于多送了 4 个字节：机床把那 4 个字节当成了数据，
于是写进去的是"长度 + 数据"，看着就像"没写进去"。

改法：**载荷里只放数据**（`[每个点 1/2 字节]`），长度交给块头末尾那两格（驱动的
`data` 机制本来就会把载荷长度写进 `tag1`）。

改完实测（同一台模拟器，客户端自己写、自己读回复核）：

| 族 | 写 | 结果 |
|---|---|---|
| `X` 0..1 | 0x40,0x41 | ✓ 读回 `[64,65]`，隔 1 秒还在 |
| `G` 0..1 | 0x40,0x41 | ✓ |
| `K` 0..1 | 0x40,0x41 | ✓（保持继电器，正是"肯定有能写的"那种）|
| `D` 300..301（字）| 0x0100,0x0101 | ✓ 读回 `[256,257]` |
| `R` 0..1 | 0x40,0x41 | ✓（R0 上被梯形图盯着的位会被改回去，比如写 0x33 读回 0x32）|
| `Y` 0..1 | — | ✗ 机床直接回块错误（输出是梯形图驱动的）|
| `R` 0..3（4 个点一次）| — | ✗ 本机拒（与读那侧同一个脾气：一次只认一个点）|

两条口径：
* **写后复核照样在**：读回来对不上就**逐个号重写一遍**（这台机床一次只落一个点），
  还是一个都落不下才回 `NCL_ERR_UNAVAILABLE`，**绝不回假成功**；
* 适配器把 `set_value` 开在 **`@X @G @R @K @D`** 上（本机实测能写），
  **`@Y @F` 保持只读**（`Y` 这台机床拒写、`F` 是 CNC 驱动的信号），写权限在适配器外面控。

### 11.23 PMC 各族的真实号段：`pmc_rdpmcinfo` = **0x8003**（2026-09-23）

上一节 `get_length` 用的还是 spec 上 0i-D 那版的范围（**手抄**的），这节把它换成
"**问机床**"：`pmc_rdpmcinfo(h, -1, ODBPMCINF*)` 一条调用把各族实际支持的号段全拿回来。

```
请求   func 0x21，一个块：code = 0x8003、块头第 2 格 = 2、d = -1（全族）
应答   载荷 = [记录数 BE32] + 64 × [族字母 2 字节][属性 2 字节][起始号 BE32][结束号 BE32]
```

记录**不是连续排的**（一族可能占好几段），所以 client 出门是：

```json
{"G":{"blocks":[[0,767],[1000,1767],…,[9000,9767]],"count":7680}, "R":{…,"count":8500}, …}
```

实测（这台 0i-MF，与 spec 0i-D 那张表**逐段对得上**）：

| 族 | 号段（字节） | 合计 | 位/字 |
|---|---|---|---|
| G / F | `0..767`、`1000..1767` … `9000..9767`（10 段）| 7680 | 61440 位 |
| X / Y | `0..127`、`200..327`、`400..527`、`600..727`、`1000..1127` | 640 | 5120 位 |
| R | `0..7999` + `9000..9499` | 8500 | 68000 位 |
| A | `0..249` + `9000..9249` | 500 | 4000 位 |

适配器的 `get_length` 现在**先问机床**（`ncl_focas_pmc_info()`），问不到才退回文档值；
`ncl_focas_pmc_read/write` 那边照旧：地址越出机床的段，机床自己会拒，客户端如实上报。

#### 11.23.1 定时器 / 计数器（`T` / `C`）也接上了

这两族在 FOCAS 里**没有单独的调用**（`pmc_rdpmctm`/`pmc_rdpmccnt` 这份 SDK 里根本没有），
就走同一条 `pmc_rdpmcrng`，族号 `6 = T`、`8 = C` ✓ —— 所以客户端一行都不用加。

口径上要说清楚的一点：FOCAS 里一个定时器/计数器是**两个字**（`[值][类型/设定值]`），
点位这一格给的是**值**那个字；要看类型/设定，读下一个号
（`ncl_focas_pmc_read(f, 'T', n * 2 + 1, 1, 1, …)`）。

实测（这台模拟器）：`T`/`C` 读得到（`rc=0`，这台没用到所以是 0），**也写得进**
...
#### 11.24.2 把这三条啃到"帧+参数"都清楚（2026-09-23）

**`pmc_rdcntldata`**（读 PMC 数据表 D 的控制数据）—— 码 **0x8004**：

```
入参   s_number / e_number = 组号（**从 1 起**，s=0 回 EW_NUMBER=3）
       length = **IODBPMCCNTL 结构体字节数**（线上不传；给 14/16/22/24/30/32 都行）
       # 一开始按"要几条"猜 → EW_LENGTH=2，绕了一圈
请求   code=0x8004、块头第 2 格 2、载荷 = [s][e][0][0]
应答   返回码 0、8 字节：`00002710 00000000`
       （官方库自己解出来的是 `datano_s=1、datano_e=1、data_size=0x2710=10000、
       data_dsp=0` —— 这台的 D 表这一组"大小 10000"）
```

**`pmc_rdcntlexrelay`**（扩展继电器控制数据）：**同一个签名、同一个形状**，
`(1, 1, 16)` 一样回 `rc=0`（官方库解出的 `data_size` 也是 10000）。码待抓（下一轮顺手）。

**`pmc_rdalmmsg`**（PMC 报警文本）—— 码 **0x8010**：

```
入参   type：枚举出来 **1（2 也收）**；-1/0 回 EW_NUMBER=3
       num = 起始报警号（给 0 回 EW_DATA=5 → 得给一个真的号）
       nmsg = 要几条
请求   code=0x8010、块头第 2 格 2、载荷 = [type=1][起始号=1][0][0]
应答   返回码 0、8 字节：`00000000 ffffffff`（这台**没有 PMC 报警** → 号 0 + 全 f 的哨兵）
       有报警时按 spec 是 `ODBPMCALM` 的 128 字节文本
```

**这一节的净结论**：三条的**码与入参语义都定了**（`0x8004` / `0x8010`，以及
...
### 11.25 **写参数通了**（2026-09-23，用户打开 PWE 之后）

`cnc_wrparam` 是**两条帧**，但真正管用的是**第二条**。这一节是**逐字节核过、写进去又
读回核过**的口径（NCGuide 0i-MF + 官方 Fwlib64 对照抓帧 + 我们自己的 client 复核）。

#### 11.25.1 那条 264 字节的记录

第二条帧 `0x8e` 的载荷 = **机床自己那条读应答（`0x8d`）的原样回填**，只把值那一格换掉：

| 偏移 | 内容 |
|---|---|
| `@0..4` | 号（BE32） |
| `@4..6` | **轴号**（BE16；无轴 = 0） |
| `@6..8` | 属性 `prm_type`（BE16：bit0..1 类型 0=bit/1=byte/2=word/3=2字，bit2=带轴，bit5=写保护） |
| `@8..12` | **值**（BE32，有符号；bit/byte 参数也是整字节写） |
| `@12..264` | 其余照读应答抄回去 —— 实测补 0 与抄原样**机床都收** |

块头（那块 28 字节）：`code = 0x8e`、**`d/e/arg2/arg3` 全 0**（号与属性都在载荷里）、
`tag1 = 264`（载荷长度压在块尾那两格，§11.22.1 同一条规矩）。

机床的返回码（抓帧实测）：`0` = 收下；`2 = EW_LENGTH`（载荷不是 264 字节）；

`3 = EW_NUMBER`（记录里的号不对）；`4 = EW_ATTRIB`（轴号/属性那两格不对）。

#### 11.25.2 官方 SDK 在这台机器上**自己写不动**

对着官方库抓帧，`cnc_wrparam` 的载荷有两处对不上：

  * `length`（`4 + 字节数 × 轴数`）**根本不上线**，只是它本地校验用的；
  * 它把 `IODBPSD.type` 那个 short **原样**塞进 `@4..6` —— 那是**轴号**那一格 ——
    所以机床回 `EW_ATTRIB`；
  * 换了老口径（`type = 0`、`length = 5`）之后，它又把**号按主机序**写进记录
    （`37 1a` 而不是 `1a 37`）→ 机床回 `EW_NUMBER`。

所以 client 这一条**不是照抄 SDK 的帧**，是按机床自己的读应答形状做的。

#### 11.25.3 三个坑（都踩过）

  1. **不说"两条帧缺一不可"**：第一条 `0xa0`（问属性）是老实现的自作主张，记录里本来
     就要带轴号和属性；只发第一条时机床回码 0 但值**不动**（这就是"回码 0 却没变"的来源）。
  2. **读的那一侧：`arg2` = 轴号**。官方 `cnc_rdparam(h, number, axis, length, IODBPSD*)`
     里上线的只有 `number`（`d = e = 号`）和 `axis`（`arg2`）。无轴参数必须给 0；
     带轴参数（1320/1420/1825…）给 0 机床直接回"块返回码非 0"，给 1..n 才读得到。
  3. **写后立刻读回是旧值**：这台机器的 FOCAS 服务端对参数块有个几百毫秒的缓存
     （实测：写 4321 后立刻读 = 5555，**300 ms** 后再读 = 4321）。客户端的复核因此做成
     **轮询**（5 次 × 150 ms），一直对不上才如实回 `NCL_ERR_UNAVAILABLE`。

#### 11.25.4 客户端与适配器

  * client：`ncl_focas_parameter_axis()`（按轴读，出门带 `number/axis/type/value/raw`）、
    `ncl_focas_parameter_write()` / `ncl_focas_parameter_write_axis()`（写 + 读回复核）。
  * 适配器：`/CONTROLLER/PARAMETER` 开了 **`set_value`** —— `{"keys":6711,"value":1234}`，
    带轴参数再给 `"axis"`；`keys` 读、不给 keys 还是老样子（逐号扫 0..64 那张表）。
    权限、白名单、二次确认都在适配器外面（用户口径："权限在外面控制"）。

**实测记录**（2026-09-23，NCGuide 0i-MF，走 client 的公开 API）：

| 参数 | 是什么 | 过程 | 结果 |
|---|---|---|---|
| 6711 | 加工件数（2 字、无轴） | 0 → 4321 → 0 | 回码 0、读回逐次核对一致 |
| 6712 | 加工总件数（2 字、无轴） | 7 → 0 | 同上 |
| 1 | bit 参数（无轴） | 0 → 1 → 0 | 同上（整字节写） |
| 1320 轴 1 | X 行程限位（bit、带轴） | 999999999 → 999999998 → 999999999 | 同上，**轴 2 没被动过** |
| 20000 | 不存在的号 | 写 | 机床回"块返回码非 0"，如实报错 |

**待真机复核**：PWE 关着时应当回 `EW_PROT = 7`（这边只核到"机床拒收就如实回错"这条路）；
锁定参数（>9000 一类）同理。
