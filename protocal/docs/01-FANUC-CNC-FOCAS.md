...
### 11.25 **写参数通了**（用户把 PWE 打开之后，2026-09-23）

用户口径："参数写开关打开了"（参数写入允许 / PWE）。开了之后立刻试出来了 ——
原来卡住的两层原因都在这：**① 没开 PWE；② 写参数是"两条帧"，不是一条**。

官方库的 `cnc_wrparam(handle, 8, IODBPSD{datano=6711, type=0, data=12345})` → **`rc=0`**：

```
第 1 条   >> func 0x21：code = 0xa0、d = 参数号(6711)、e = 1、**不带载荷**（体 30）
          << 块体 44：机床把**这个参数的属性表**回回来
             （`1a36 0000 1a38 0000 1a37 0003 …` —— 前后号与"号 + 类型"）
第 2 条   >> func 0x21：code = **0x8e**、**载荷 264 字节**（就是参数那条记录的形状）：
             [0][0][0][0][0x108 = 264][datano = 0x1a37][type = 3][value = 0x3039] + 补 0
          << 18 字节、返回码 0 ✓
```

两条参数一变就错（都试过）：

* `length` 不是"值"也不是"条数"，是 **`IODBPSD` 结构体字节数**（8；给 12 → `EW_LENGTH=2`）；
* `type` 只能给 **0**（给 1/2 → `EW_RCODE=4`：那两组是"按机床属性表里的 type 自动选"的，
  给了反而被拒 —— 所以第 1 条 `0xa0` 的问属性那一步**不能省**）。

**所以 client 的实现路子**（还没写完，时间用尽：

1. `0xa0` 先问属性（d = 参数号、e = 1、无载荷）→ 拿到"这个号存不存在、是几字节、type 是几"；
2. 照 `RDPARAM`（`0x8d`）读回来的那条 264 字节记录，把**值**那几格换掉；
3. 用 `0x8e` + 那 264 字节载荷发出去 → 返回码 0 才算成；
4. 写完**读回复核**（比原始整数），对不上如实回 `NCL_ERR_UNAVAILABLE` —— 与写刀补/宏变量/PMC 同一个口径。

#### 11.25.1 客户端的写参数：两条帧都发了，**值还没落地**（2026-09-23 收工前）

按 §11.25 那三条动作写进 client 了（`ncl_focas_parameter_write`）：

1. `0xa0` 问属性（`d = e = 号`、无载荷）→ 载荷里"号"那一格**后面 2 字节**就是 `type`
   （这台 6711 → `3`）；
2. 组 **264 字节**载荷：`@16 = 264`、`@20 = 号`、`@24 = type`、`@28 = 值`（都大端）；
3. `0x8e` + 这条载荷发出去，然后**读回复核**（比原始整数），对不上如实回
   `NCL_ERR_UNAVAILABLE`。

**现状：值还没落**（`rc=-15`），两处已经改对、还差最后一处：

| | 官方库（能落）| 我们（改之前）|
|---|---|---|
| 块头四格 `d/e/a2/a3` | **全 0** | `d = 号、e = 1` ✗ → **已改成 0** |
| 载荷 @16 | `0x108 = 264` | 264 ✓（原来那版偏移写错，已修）|
| 载荷 @20/@24/@28 | 号 / type / 值 | ✓ |

改完这两处之后**还是没落**（收工前最后一次实测：写 6711 = 6789 → `rc=-15`；读回仍是
12345 —— 那个 12345 是**官方库写进去的**，证明这台机器开了 PWE 之后写参数确实能落）。

**下一轮第一件事**：把我们的 `0x8e` 帧与官方库那次能落的帧**再逐字节 diff 一次**
（工具都在：`cap.py` + `focas_pmc.exe` 里的那段写参数试跑）；差的那一处就在载荷
@0..16 那 16 个 0 或者 `0xa0` 那条应答的用法上。**在写通之前，适配器不给"参数可写"
的声明**（现在 `parameter_write` 只会如实回错，不会假成功）。
