...
`//CNC_MEM/USER/PATH1/O0001` 这种文件路径，机床在 start 那一步就回 `EW_ATTRIB=5`
（SDK 同样被拒）。程序号不从路径来，从正文第一行来（也就是上面那条口径）。
...
```
a0a0a0a0 0003 13 03 0008  | 00 00 00 05 00 01 00 00
            ^func ^dir=3    ^^^^^^^^^^^ 前 4 字节 = 返回码：5 = EW_ATTRIB
```
...
方向 3 + 码 0 = 这一步成了；码非 0 = 翻成 `NCL_FOCAS_ERR_TRANSFER(码)`，
`last_error` 里出人话（`机床回码 5（EW_ATTRIB（属性/数据不对，机床不收））`）。

#### 11.14.3 这台模拟器上：上下行**都到不了**（SDK 一样）

| 动作 | 官方 SDK 的结果 | 说明 |
|---|---|---|
| 下行（目录 + 真程序正文）| start `rc=0` → download4 `rc=0`（24 字节发出去）→ **end `rc=5`** | 机床在 end 上说 `EW_ATTRIB`：这份数据不收，程序目录里也不会多出来 |
| 下行（旧试探：正文铺 'A'×1024）| end `rc=0` | 只有"不成程序"的那份它才回 0（其实什么都没写进去）|
| 上行（已存在的程序 O2001/O3000…）| start `rc=0` → **upload4 `rc=10`** | `0x18 dir 4` 的**数据请求机床一声不响**（SDK 那份也一样）—— 应答里程序文本的切法在这台机器上取不到 |

所以"程序上行"这一格仍旧是 `NCL_ERR_UNAVAILABLE`，但**缺的东西更清楚了**：请求帧
（`0x15` 与下行同形、`0x18 dir 4` 体 8 字节）已经核过，差的是**应答里文本的切法**，
而它要一台肯答 `0x18` 的机床（真机）才能定。

#### 11.14.4 没解决的一条（如实记在这儿）
...
| 流式下行 `cnc_dwnstart4`（格式已核：`\nO<号>\n…\n%`）| SDK 完全通、程序真进机床 ✓；**本 client 发的 start 帧被机床关连接** ✗ |

最后那条的排查（§11.14.4 有全表）这轮又排除了两条：

* **并行**跑两个官方 SDK 下载 → **两个都成功** ✓ → 机床**不是**"同一时刻只服务一个客户端"；
* 下载前先做一次**文件族读**（0xf0）/ **建文件**（0xb5）/ **写刀补**（0x09）→ 都不影响（还是被关）
  → 不是"要先做点什么把传输点着"。

也就是说：**"能不能传"确实落在"这条会话是怎么建起来的"**（帧逐字节相同也没用），
下一步得从 TCP 层面比（SYN 选项/窗口）或者在 guest 里抓包 —— 主机这边 `pktmon` 要管理员
权限，拿不到。
