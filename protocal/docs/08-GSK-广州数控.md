...
**数据项 12/12 全通**。还差的只有 `cmd` 写命令那条路（`Init` 已经能看到帧
`93 00 0e 00 | 65 c8 0c 64 0c 01 00 00 3f 03 00 01 00 00 | 55 aa`）。

写路径试过一轮（`bind_init_probe.sh`）：按 `cmd` 那条路造应答（期望头
`65 64 0c c8 0c <sn> 00 00`、载荷 ≥ 12 字节）仍然报
`unexpected response data`——所以它还差一层校验（不是在 `shouldResponseCmd`
的壳检查上，得再往下读 `(*GskCnc).Init` `0x61ac0c` 的返回解析）。
