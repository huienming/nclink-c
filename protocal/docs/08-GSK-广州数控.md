...
> ⚠️ **地址表是关键缺口**（**仅通道 B 需要**）：没有厂商地址表就无法确定哪个寄存器是哪个数据。
> 需向代理商索取，或现场用扫描法反推（不推荐，风险高）。
> **2026-09 清账：本包不用这一路** —— 现场盒子对 GSK 走的是**通道 A 的 HTTP 路**
> （`libgsk-http.so` + `gsk_mod.lua`，见 §4.5–§4.7），全机没有 `gskrm.dll`。
> 所以这条不影响交付，只在"自研上位机直连 GSK 机床"时才需要补。
...
### 4.6 应答侧（🟢 2026-09 第三轮，`tools/site-probe/gsk_reply_probe.sh`）—— 数据项 12/12
...
| `PROGRAM` | `12` | 数据 [3..4] = **u16 名字长度**、名字在数据 **[5]** 起，且**数据总长 ≥ 5+长度**（不然走错误分支回空串）→ `'O1000'` / `'ABC'` ✅ |

**数据项 12/12 全通**。

### 4.7 `Init`（`cmd` 写命令那条路）也闭环了（🟢 2026-09 第六轮，`gsk_init_probe2.sh`）

`(*GskCnc).Init`（`0x61ac0c`）反汇编读出来的过程：

```
请求载荷（6B）  3f 03 00 01 00 00
应答载荷（6B）  bf 03 00 01 00 00
                └ 第 1 字节 = 请求第 1 字节按位取反（mvn r5,#64 → 0xBF），其余 5 字节照抄
1. cmd() 建帧：93 00 | 0e 00 | 65 c8 0c 64 0c <sn> 00 00 | <6B 载荷> | 55 aa   （<sn> = getCmdSN()，逐条 +1）
2. shouldResponseCmd 校验壳 + 期望头：65 64 0c c8 0c <sn> 00 00
   （= 请求头的第 1、3 字节对调：65 c8 0c 64 0c → 65 64 0c c8 0c）
3. Init 自己再判一次：应答载荷长度必须**正好 6**，且 memequal(`bf 03 00 01 00 00`)
```

上一轮（`bind_init_probe.sh`）只做到第 2 步（头换序），所以卡在 `unexpected response data`；
把载荷第 1 字节 `3f` 改成 `bf` 就过了 —— 实测：

| 应答（echo 请求后打补丁） | 结果 |
|---|---|
| `EREP:5:64,7:c8,12:bf`（头换序 + 载荷首字节取反，6B） | ✅ `None`（成功） |
| `EREP:5:64,7:c8`（只换头，载荷仍 `3f…`） | ❌ `unexpected response data` |
| 同上 + 载荷多 1 字节（7B） | ❌ `unexpected response data`（长度必须正好 6） |

请求原样（实测 sn 逐条 +1）：

```
93 00 0e 00 65 c8 0c 64 0c 01 00 00 3f 03 00 01 00 00 55 aa
93 00 0e 00 65 c8 0c 64 0c 02 00 00 3f 03 00 01 00 00 55 aa
```
...
1. ~~**缺 SDK 本体**：`gskrm.dll` 需从广数代理/系统光盘获取~~ —— **2026-09 清账：这不是本包的缺口。**
   现场盒子走的是 **HTTP** 那条路（`app1/nclink-service/libgsk-http.so` +
   `lua/lua_mod/gsk_mod.lua` + `cfg/driver_defs/gsk_driver.json`），全机扫过一遍
   **没有 `gskrm.dll` 的任何影子**；§4.5/§4.6 的实测帧也都是 HTTP 端点抓的。
   DLL 那条路只在"自研上位机直连机床"时才需要，与本包无关。
2. **32 位限制**：DLL 为 x86，64 位采集程序需用独立 32 位进程桥接（IPC）。（仅 DLL 路线）
3. ~~**Modbus 地址表缺失**：自实现 Modbus 路线的唯一障碍~~ —— 同上，本包 GSK 不走 Modbus。
...
| **真机实测日志** 🟢 | `INCBOX200/dev/hp2x/test_manual_result/test_gsk_980TDi_20220914.log`（2022-09-14，980TDi）：`Status:0` / `FEED_OVERRIDE:150` / `SPDL_OVERRIDE:120` / `RAPID_OVERRIDE:0` / `PART_COUNT:346` / `PROGRAM:O0607` / `WARNING:[{1031 外接急停信号(X9.3)断开 PLC信息地址位:A0003.7。} {0 急停报警,ESP输入开路}]` |
| ~~待补~~ | ~~GSKRM.dll 本体 + Modbus 地址表~~ → 已清账（见 §5.1/§5.3），本包不需要 |
