...
3. **值的读法：数据段头 8 字节 = 小端 float64**（🟢 已验证）。把 `S7 R:` 模式
   ...
   | `FeedActual` | 见下 | 读数据段**偏移 0 的 double**，但回的是变换后的值 |
   | `Mode` | `"AUTO"` | 项长 **4 字节**时返回模式**名字**（整数码 → 文本） |
   | `Execution` | `error response` | 仍待定（它要 2 项，两项各自长度可能不同） |

   `FeedActual` 的变换实测：数据段偏移 0 放 `42.0`、其余为 0 → 回 **17.64**
   （= 42 × 0.42）；偏移 8 及以后放 42.0 → 回 0。说明它的输入确实在偏移 0，
   但输出经过一次换算（很可能是"实际进给 = 设定值 × 某个因子"这类业务换算，
   而不是裸读）。`Mode` 则是"4 字节整数码 → 模式名"，用不同的 4 字节值扫一遍
   就能把码表补全。
