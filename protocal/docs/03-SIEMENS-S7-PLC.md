...
也就是说**传输层与握手已经完全对上**（网关不再报超时/EOF，而是读到了应答）。
剩下的"最后一格"是它判 `error response`——多半出在应答里的**返回码/PDU 协商字段**
（我们按 S7 标准填 `0xFF` 成功、PDU 480）。拿一次真机抓包对着改，或继续试这几个
字段即可。数据块里放什么值就能读出什么，所以**一旦这一格通了，S7NCU 的 25 个
数据项一次就能全定下来**（`S7S:<值的 hex>` 改一下就行）。
