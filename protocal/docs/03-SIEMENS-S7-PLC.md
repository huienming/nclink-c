...
值的读法也看清了：把值塞进数据块后，网关回的是一个小端 double，其 8 字节位型
与我们喂进去的字节一一对应：

```
数据 40 45 00 00 00 00 00 00  →  bits 00 00 45 40 08 00 04 ff
数据 12 34                    →  bits 00 00 34 12 10 00 04 ff
数据 00 2A                    →  bits 00 00 2A 00 10 00 04 ff
```

（后 4 字节 `08 00 04 ff` 正是那个数据项头 `FF 04 00 08` 的小端读法——它把
"项头 + 值"整段按结构体读了。）所以**只差把 double 对准到正确的偏移**：下一轮用
"整段唯一值 + 看返回位型"就能定位，25 个数据项随即可一次定下来。
