 ...
 *
 * A point that wants a field *inside* a block writes the byte offset into the
 * item name, because the generic address model reserves `bit` for a bit index
 * (any `bit` in a configuration becomes dtype BIT):
 *
 *   {"path": "/CNC/STATUS@ALARM",
 *    "addr": {"area": "STATINFO@12", "dtype": "int16"}}   // ODBST.alarm
 *   {"path": "/CNC/AXIS@1/POSITION",
 *    "addr": {"area": "ACTF@4", "dtype": "float32"}}      // second axis
 *
 * A name without `@`, or one whose tail is not a decimal number, is taken
 * whole (`"AXIS@0/SCREW"` style names from other protocols stay intact).
 *
