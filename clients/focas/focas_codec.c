...
static const ncl_focas_item kItems[] = {
     ...
     * `0x8d`、`d = e = 参数号`；0x0e 那条在这台机器上被拒 rc=1）。
     * **`arg2` = 轴号**（0 = 无轴；带轴参数 1320/1420/1825… 必须给 1..n，给 0 机床
     * 回"块返回码非 0"）—— 官方 `cnc_rdparam(h, number, axis, length, IODBPSD*)` 那个
     * `length` 根本不上线。应答 264 字节 = 参数记录（§11.26）：
    ...
    /*
     * 按文件名按行读程序（`cnc_rdpdf_line`）= **`0xf0`**：`d = 起始行号`（程序头是 0）、
     * `e = 读几行`、**载荷 = 程序路径**（256 字节、NUL 补齐，`tag1` = 256、块长 = 28+256）。
     *
     * 帧是 2026-09-23 用官方 SDK 对 NCGuide 0i-MF 抓的（01 册 §11.16）：请求
     * `Cb 0xf0 d=0 e=32 tag1=256`+载荷 `//CNC_MEM/USER/PATH1/O3001`，应答体就是**程序正文**
     * （384 字节、`O3001(SUBPOCKET)\n…\nM99\n%`）。路径给裸文件名（`O3001`）机床回
     * `EW_DATA`（细码 1 = 程序路径错）—— 要"盘名+路径+文件名"。
     *
     * ⚠️ 官方手册把这条标成 **HSSB 专用**（以太网列是 `-`），可这台模拟器的以太网照答
     * （真机上未必）—— 用的时候要留退路，见 §11.16。
     */
    { "RDPDFLINE",  { 0xf0, 0, 0 },      { 0, 0, 0 },      { 64, 0, 0 },     1, false },
    /*
     * 建程序文件 / 文件夹（`cnc_pdf_add`）= **`0xb5`**：`d` = 类型（0 = 文件 / 1 = 文件夹）、
     * 载荷 = 256 字节路径（与 `RDPDFLINE` 同形）。
     *
     * 2026-09-23 实测（模拟器）：`//CNC_MEM/USER/PATH1/O1234` 建出来 `rc=0`，而且机床
     * **自动把程序号那一行写进去**（读回来是 `O1234\n`）。
     */
    { "RDPDFADD",   { 0xb5, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false },
    /*
     * 删程序文件 / 文件夹（`cnc_pdf_del`）= **`0xb6`**：载荷 = 256 字节路径
     * （与 `RDPDFLINE`/`RDPDFADD` 同形）。2026-09-23 实测：`//CNC_MEM/USER/PATH1/O1234`
     * 删掉 `rc=0`；路径不存在时回 `EW_ATTRIB`（上一轮就是拿不存在的路径试的，误判成"码不对"）。
     */
    { "RDPDFDEL",   { 0xb6, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false },
    /*
    ...
    /*
     * 伺服负载电流（安培）：**还是 0x56**，`d = 3`（`d = 1` 是负载表，见上面那条）。
     * 官方 SDK 的 `cnc_rdaxisdata(cls = 2 Servo, type = 2)` 发出来就是这一格
     * （2026-09-23 抓帧，01 册 §11.19）；记录形状与负载/位置那一族相同。
     */
    { "SVCURRENT",  { 0x56, 0, 0 },      { 3, 0, 0 },      { 0, 0, 0 },      1, false },
    /*
     * 指令值（`cnc_rdcommand`）：一个 **0x97**，`d` = 类型（-1 = 全部模态非 G 码、
     * -2 = 全部指令值、100..129 逐条），`e` = 1（第几块）。应答 = 12 字节一条记录：
     * `[adrs(1)][num(1)][flag(2)][cmd_val(4 BE)][dec_val(4 BE)]`，`adrs` 是字母
     * （`'T'` 0x54 = 刀号）。**当前刀号就在 `'T'` 那条的 `cmd_val`**（01 册 §11.19）。
     */
    { "RDCOMMAND",  { 0x97, 0, 0 },      { 0xffffffff, 0, 0 }, { 1, 0, 0 }, 1, false },
    /*
     * PMC（FANUC 的 PLC 就是 PMC）：`pmc_rdpmcrng` = **0x8001**、`pmc_rdpmcinfo` = 0x8003
     * （2026-09-23 抓帧，01 册 §11.21）。载荷四格是**号段与族**，顺序固定：
     *
     *     [起始号 BE32][结束号 BE32][族 adr_type BE32][数据宽度 data_type BE32] + 数据区
     *
     * 族：0=G、1=F、2=Y、3=X、4=A、5=R、6=T、7=K、8=C、9=D；
     * 宽度：0=字节、1=字、2=长字。**结束号 = 起始号 + 要几个**（spec 的例子：
     * `D0100` 字型 → `s=100 e=101 length=10` 读 1 个）。读回来的应答体就是数据（1/2/4
     * 字节一个点，大端）。
     */
    /*
     * 写参数（`cnc_wrparam` 的**第二条帧**）= **0x8e**，载荷 = 机床自己那条 264 字节
     * 读记录**原样回填、只换值那一格**（2026-09-23 写进去又读回核过，01 册 §11.26）：
     *
     *     @0..4  号（BE32）、@4..6 轴号（BE16）、@6..8 属性 prm_type（BE16）、@8..12 值（BE32）
     *
     * 块头四格 `d/e/arg2/arg3` **全 0**（号与属性都在载荷里）、`tag1` = 264；载荷给
     * 12/8 字节机床回 `EW_LENGTH=2`。写完要**等几百毫秒**再读回（机床对参数块有缓存），
     * 客户端那边做成轮询再复核（`ncl_focas_parameter_write_axis`）。
     */
    { "WRPARAM2", { 0x8e, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    { "PMCRNG",  { 0x8001, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    /*
     * PMC 写（`pmc_wrpmcrng`）= **0x8002**（读 + 1）。同一个四格载荷，末尾再跟
     * `[数据长度 BE16][数据…]`（2026-09-23 抓帧，01 册 §11.22）：
     *
     *     >> code=0x8002 [s=0][e=1][族=5][宽度=0] [0002]['AA''55']   → 写 R0..R1
     *     << 块 16 字节、返回码 0
     */
    { "PMCWR",  { 0x8002, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    /*
     * PMC 各族**实际支持**的号段（`pmc_rdpmcinfo`）= **0x8003**，`d = -1` 全族。
     * 载荷 = `[记录数 BE32]` + 64 × `[族字母 2 字节][属性 2 字节][起始号 4][结束号 4]`
     * （2026-09-23 实测：G 一族有 10 段，`0..767`、`1000..1767` … `9000..9767`，
     * 与 spec 0i-D 那张表对得上，见 01 册 §11.23）。
     */
    { "PMCINF",  { 0x8003, 0, 0 },      { 0xffffffff, 0, 0 }, { 0, 0, 0 }, 1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    /*
     * PMC 的参数区与报警（2026-09-23 抓帧，01 册 §11.24）：
     *   `pmc_rdcntldata`       = **0x8004**（数据表 D 的控制数据，载荷 `[s][e]`，组号**从 1 起**）
     *   `pmc_rdcntlexrelay`    = **0x8057**（扩展继电器控制数据，同一个形状）
     *   `pmc_rdcntlgrp`        = **0x8006**（数据表控制数据有几组）
     *   `pmc_rdcntl_exrelay_grp` = **0x8059**（扩展继电器控制数据有几组）
     *   `pmc_rdalmmsg`         = **0x8010**（PMC 报警文本，载荷 `[type][起始报警号]`，
     *                            `type` 用 **1**；起始号给 0 机床回 EW_DATA）
     */
    { "PMCCNTL", { 0x8004, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    { "PMCEXRLY",{ 0x8057, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    { "PMCGRP",  { 0x8006, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    { "PMCEXGRP",{ 0x8059, 0, 0 },      { 0, 0, 0 },      { 0, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    { "PMCALM",  { 0x8010, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                 { 0, 0, 0 },          { 0, 0, 0 } },
    /*
    ...
    { "ALMMSG",     { 0x23, 0, 0 },      { 0xffffffff, 0, 0 }, { 10, 0, 0 }, 1, false,
                     { 2, 0, 0 },        { 64, 0, 0 } },
    /*
     * 工件零点偏移（`cnc_rdzofs`）：一个 **0x0b**，四格是 `d` = 偏移号、`e` = 偏移号
     * （单条读照老规矩 d = e = 号）、`arg2` = 轴号（**-1 = ALL_AXES**）、`arg3` = 0。
     * 2026-09-23 用官方 SDK 对 NCGuide 0i-MF 抓的帧：
     *
     *     >> code=0x0b [00000001][00000001][ffffffff][00000000][00000000]
     *     << 载荷 256 字节 = **32 条 8 字节记录**（MAX_AXIS = 32）
     *
     * 一条记录 = `[值 BE32][辅助字 BE32]`，值就是偏置量（最低输入单位，小数位见
     * `cnc_getfigure`）。单轴读（`arg2 = 1`）只回**那一条** 8 字节 —— 所以
     * 第 i 根轴的值 = 载荷 `@8×(i-1)`。
     *
     * 写（`cnc_wrzofs`）官方库在这台机器上自己先挡了（回 EW_LENGTH、一帧不发），
     * 码按"读 + 1"推是 **0x0c**，载荷形状还没核（见 `WRZOFS`）。
     */
    { "RDZOFS",     { 0x0b, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                     { 0xffffffff, 0, 0 }, { 0, 0, 0 } },
    { "WRZOFS",     { 0x0c, 0, 0 },      { 1, 0, 0 },      { 1, 0, 0 },      1, false,
                     { 0xffffffff, 0, 0 }, { 0, 0, 0 } },
