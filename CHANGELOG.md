...
### 01 册：写参数——两条帧都发了，值还没落地（收工前最后一次尝试）

  * 按 §11.25 的三步实现进 client：`0xa0` 问属性（拿 `type`）→ 组 264 字节载荷
    （`@16=264`、`@20=号`、`@24=type`、`@28=值`）→ `0x8e` 发 → 读回复核。
  * 逐字节比了帧，改对两处：**块头四格必须全 0**（早先写了 `d=号、e=1`）、载荷里那几格
    的**偏移**（原来 264/号/type/值的位置全错）。
  * 改完**还是没落**（写 6711 = 6789 → `rc=-15`；读回仍是 12345 —— 那是**官方库**写进去
    的，说明这台机器开了 PWE 之后写参数确实能落）。
  * 下一轮第一件事：把我们的 `0x8e` 与官方库能落的帧**再逐字节 diff 一次**（差在载荷
    @0..16 那 16 个 0，或 `0xa0` 那条应答的用法）。**写通之前不声明"参数可写"**；
    `ncl_focas_parameter_write()` 现在只会如实回错，不会假成功。

### 01 册：写参数**通了**（PWE 打开 + 两条帧）

  * 用户把**参数写入允许（PWE）**打开后，官方库 `cnc_wrparam(h, 8, IODBPSD{6711,0,12345})`
    立刻回 **rc=0** —— 之前卡住的两层原因：**没开 PWE** + **写参数是两条帧**。
  * 帧：① `code = 0xa0`、`d = 参数号`、`e = 1`、无载荷 → 机床回**该参数的属性表**；
    ② `code = 0x8e` + **264 字节载荷**（参数记录形状：`[…][264][号][type][值]`）→ 返回码 0。
  * 两个坑：`length` 是 `IODBPSD` 的**结构体字节数**（8，给 12 → EW_LENGTH）；`type` 只给
    **0**（给 1/2 → EW_RCODE=4，所以第 1 步问属性不能省）。
  * client 实现路线记进 §11.25（0xa0 问属性 → 拿 0x8d 的 264 字节记录改值 → 0x8e 发 →
    读回复核），**这一轮没写完**（时间用尽）。
