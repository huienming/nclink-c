...
### 新代 SYNTEC 适配器：九项按 10 册 §3.1/§3.2 的现场闭环实现
  ...
  * **轴名读到了：在参数区（0x0404）**（2026-09-22）：
    轴名不在状态区，在**系统参数区**——同一个帧形状、另一个请求号
    （控制器侧 `OCK_CODE::CODE(type, id) = (type << 10) | id`）：`0x0401` 容量、
    `0x0402` 参数表（`TParamSpec`，21A 3784 条）、`0x0404` 读一个参数（**i32**，
    与状态区的 int16 不同宽），`0x0407` 就是我们一直在用的状态区。
    轴名在 `321 + 槽`（`*Nth axis axis name`），`21 + 槽` 是端口号（0 = 这一槽没接轴），
    `221 + 槽` 是轴型；解码抄客户端 `get_AllAxisName()`：`代号 / 100` 是 `"XYZABCUVW"` 的位置，
    `代号 % 100` 是后缀（0 与 >= 10000 = 这一槽没名字）；“哪些轴在用”抄
    `get_EnableAxisMappingID()`（端口 > 0 且 0 < 代号 < 10000）。
    client 侧新增 `ncl_syntec_param()` / `ncl_syntec_axis_name()` /
    `ncl_syntec_axis_name_decode()` / `ncl_syntec_axes()`；适配器侧新增方法 `/AXES`
    （REST：`POST /api/syntec/AXES`）。**21A 实测**：`/AXES` 答 4 条轴
    `X(槽1,口1) / Y(槽2,口1) / Z(槽3,口3) / C(槽6,口6)`，`stateCount = 6`
    与状态区 261 的 6 项对得上；`--once` 仍 **17/17 可读、0 失败**；mock 用例
    证了解码与“一条轴都没有就回 `NCL_ERR_UNAVAILABLE`”。
    **记一处不一致**：声明里原按 `X=0、Z=1` 写死，
    而这台控制器说位置下标 1 是 `Y`。改法见下一条：点位 `arg` 传轴字母、
    读值时现查轴号，不再靠声明顺序（ 10 册 §11.4）。
  * **路径写死、轴号现查**（2026-09-22）：点位路径照旧是 iNC-BOX 的 `/AXIS@<名>/...`
    ...
    实测：这台 `X` → 槽 0、`Z` → 槽 2（声明里 Z 是第二个点 = 下标 1，会读错轴）；
    mock 把状态区摆成 `{1234, 777, -500}`，Z 必须落在下标 2 才读到 -500 ✓；
    21A `--once` 仍 **17/17 可读、0 失败**。
