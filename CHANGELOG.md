...
### 新代 SYNTEC 适配器：九项按 10 册 §3.1/§3.2 的现场闭环实现
  ...
  * **路径写死、轴号现查**（2026-09-22）：点位路径照旧是 iNC-BOX 的 `/AXIS@<名>/...`
    ...
    -500 ✓ —— 证明轴号是现查的，不是声明顺序。
  * **轴按五轴配全**（2026-09-22）：位置点位从 X/Z 扩到 **X/Y/Z/A/C 五轴 × 4 组 = 20 个**
    （与 FANUC 适配器同一套格子，这样一个模型能套各种机型）；机器没配的轴（控制器轴表里
    没这个名字）照实报错，不给个数、也不去读别的轴。实测：21A `--once` 下 **29 个点位里
    25 个读得到**（这台运行配了 X/Y/Z/C），`/AXIS@A/...` 四组全部 `NotFoundException`；
    mock 用例里 Y/A/C 也都报错。
    位置点位：一个模型能套各种机型）；机器没配的轴（控制器轴表里没这个名字）
    照实报错，不给个数、也不读别的轴。实测：21A `--once` 下 **29 个点位里 25 个读得到**（这台运行
    配了 X/Y/Z/C），`/AXIS@A/...` 四组全部 `NotFoundException`；mock 用例里 Y/A/C 也都报错。
