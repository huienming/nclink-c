                              ...
                              ncl_json **value)
{
    ...
    err = focas_build_item(address->area, body, sizeof(body), &used);
    if (err != NCL_OK) {
        return err;
    }
    ...
    if (address->offset < 0 || (size_t)address->offset >= ncl_focas_block_count(reply, reply_len)) {
        return NCL_FOCAS_ERR_RB_MISSING;
    }
    err = ncl_focas_block_at(reply, reply_len, (size_t)address->offset, &block,
                             &block_len);
    if (err != NCL_OK) {
        return err;
    }
    ...
    payload_offset = address->bit > 0 ? (size_t)address->bit : 0u;
