...
`get_value#<模型路径>`；`/CNC/SESSION`、`/CNC/ITEMS` 两个方法可以问会话状态与点表。
...
|---|---|---|---|
| `/CNC/STATUS@MANUAL` `@EDIT` | `STATINFO@0` `@4` | 手动 / 编辑方式 | ✔ |
| `/CNC/STATUS@RUN` `@MOTION` `@MSTB` | `STATINFO@2` `@6` `@8` | 运行 / 轴运动 / 移动中 | ✔ |
| `/CNC/STATUS@EMERGENCY` | `STATINFO@10` | 急停 | ✔ |
| `/CNC/STATUS@ALARM` | `STATINFO@12` | 报警中（0 = 无报警） | ✔ |
| `/CNC/STATUS@SPINDLE` `@OPERATOR` | `STATINFO@14` `@16` | 主轴 / 操作相关状态 | ✔ |
| `/CNC/STATUS@DUMMY` `@AUTO` | `STATINFO` + `offset` 1 / 2 | 保留字段 / 自动方式 | ✔ |
| `/CNC/PART_COUNT` | `RDCOUNT` | 加工计数（int32） | ✔ |
| `/CNC/PROGRAM@NAME` | `EXEPRGNAME2` | 当前加工程序名（36 字节字符串） | ✔ |
| `/CNC/AXIS@k/POSITION` | `ACTF@<4k>` | 第 k 轴位置（k=0 即第一轴，一般是 X） | ✔ |
| `/CNC/AXIS@k/SPEED` | `ACTS@<4k>` | 第 k 轴速度 | ✔ |
