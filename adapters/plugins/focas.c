 ...
 * 一律按需读 —— 要上报就把那一行的 NCL_DATAITEM 换成 NCL_DATAITEM_SAMPLED。
 ...
 *                  所以那边用 NCL_DATAITEM_PENDING 声明：模型里有它，问它会
...
NCL_TOOL_BEGIN("focas", "FANUC FOCAS / Fwlib32 over TCP, read only", 1000, 1000,
     ...
     * 其余（位置/速度）一律按需读 —— 要上报就把 NCL_DATAITEM 换成
     * NCL_DATAITEM_SAMPLED。 */
    ...
    NCL_DATAITEM_SAMPLED("/MACHINE/CONTROLLER/PROGRAM", focas_dispatch,
                         &k_program)
    /* 报警（表 6 的 WARNING）：进默认采样通道，但帧还没抓到（01 册 §2.3 的码表里
     * 没有 cnc_rdalmmsg2），所以先占着通道、取值为 null，自检把它算成"待抓包"。
     * 抓包补上之后换成 NCL_DATAITEM_SAMPLED("/MACHINE/WARNING", …, &k_warning)。 */
    ...
    NCL_DATAITEM("/MACHINE/AXIS@X/POSITION@REAL", focas_dispatch,
                 &k_axis_x_position_real)
    NCL_DATAITEM("/MACHINE/AXIS@Y/POSITION@REAL", focas_dispatch,
                 &k_axis_y_position_real)
    NCL_DATAITEM("/MACHINE/AXIS@Z/POSITION@REAL", focas_dispatch,
                 &k_axis_z_position_real)
    NCL_DATAITEM("/MACHINE/AXIS@A/POSITION@REAL", focas_dispatch,
                 &k_axis_a_position_real)
    NCL_DATAITEM("/MACHINE/AXIS@C/POSITION@REAL", focas_dispatch,
                 &k_axis_c_position_real)
    NCL_DATAITEM_PENDING("/MACHINE/AXIS@X/POSITION@CMD",
                         "目标位置：帧待抓包（cnc_rdposition）")
    NCL_DATAITEM_PENDING("/MACHINE/AXIS@Y/POSITION@CMD",
                         "目标位置：帧待抓包（cnc_rdposition）")
    NCL_DATAITEM_PENDING("/MACHINE/AXIS@Z/POSITION@CMD",
                         "目标位置：帧待抓包（cnc_rdposition）")
    NCL_DATAITEM_PENDING("/MACHINE/AXIS@A/POSITION@CMD",
                         "目标位置：帧待抓包（cnc_rdposition）")
    NCL_DATAITEM_PENDING("/MACHINE/AXIS@C/POSITION@CMD",
                         "目标位置：帧待抓包（cnc_rdposition）")
    NCL_DATAITEM("/MACHINE/AXIS@X/SPEED", focas_dispatch, &k_axis_x_speed)
    NCL_DATAITEM("/MACHINE/AXIS@Y/SPEED", focas_dispatch, &k_axis_y_speed)
