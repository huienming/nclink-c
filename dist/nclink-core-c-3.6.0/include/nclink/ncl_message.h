...
/**
 * 把"整列 `[]`"的占位列换成等长的 null —— 设备端**发布之前**该做的第一步。
 *
 * 为什么需要它：整列 `[]` 在采样编码里是"本周期该项没有数据"的占位，而一个
 * **空表值**（报警列表为空这种）编码出来跟它**长得一模一样**。所以设备不能拿
 * "有一列是 `[]`"当作"这一包没采到"，更不能因为它把整包丢掉：
 *
 *   - 真·没采到（整窗都错过）：换成 null 照发 —— 消费端看到的是一条完整报文、
 *     里面是 null（"设备活着，这一刻没有数据"），上报节奏不受影响；
 *   - 某一项的值本身就是空表：换掉那一列（`[]` → null，语义等价："什么都没有"）。
 *
 * 外层结构（表头与列数一致、各列槽位数对齐）对不上的报文仍然发不出去，那是
 * ncl_message_sample_is_complete() 的判据，与这里无关。
 *
 * 按列处理，不看别的列是什么形状。返回换掉的列数（0 = 没有这种列；内存不足时
 * 返回已换的列数，该列会短一截，外层对齐判据会拦住它）。
 */
