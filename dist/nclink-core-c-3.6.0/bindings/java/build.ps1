...
# javac / java write warnings and log lines to stderr, and the library logs the
# SEVERE paths the negative tests deliberately drive it into. With
# $ErrorActionPreference = "Stop", PowerShell turns any native stderr byte into a
# terminating error, so the run would abort on expected noise. Relax the
# preference around the toolchain and judge each step by its exit code instead.
...
# java.util.
Invoke-Tool $javac.Source (@("-encoding", "UTF-8", "--release", "8", "-d", $classes) + $sources)
...
if (-not $NoTest) {
    ...
    Invoke-Tool $java.Source @("-Djava.library.path=$libdir", "-Dfile.encoding=UTF-8",
    ...
    if ($env:NCLINK_TEST_BROKER) {
        Invoke-Tool $java.Source @("-Djava.library.path=$libdir", "-Dfile.encoding=UTF-8",
            "-cp", $classes, "com.nclink.BrokerE2E")
