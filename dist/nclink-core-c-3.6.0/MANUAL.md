...
1. **占位列换成 null**（`ncl_message_sample_fill_empty_columns()`）：整列 `[]` 是
   "本周期该项没有数据"的占位，换成等长的 null。
2. **再校验 `ncl_message_sample_is_complete()`**：
...
**为什么必须先换**：占位 `[]` 与"某一项的值本身就是空表"（比如"当前没有报警"的报警
列表）编码出来**一模一样**。拿"有一列是 `[]`"当"这一包没采到"会把整包错杀掉 ——
通道里只要有一项可能为空表，这个通道就永远发不出去。换成 null 之后两种情形都成立：
真没采到 → 一条完整的 null 报文（"设备活着、这一刻没有数据"，上报节奏不受影响）；
空表值 → 语义等价的 null。

只有**外层对不上**（表头与列数不符、各列槽位数不齐）的窗口才真的残缺，会被
...
管内层形状。判断采样报文能否消费请一律用后者。设备端**发布前已经换过占位列**，
所以线上正常不会有整列 `[]`；收到别家实现发来的、带占位列的报文，先调
`ncl_message_sample_normalise()`（它内部就是 `fill_empty_columns` 加补表头）。
