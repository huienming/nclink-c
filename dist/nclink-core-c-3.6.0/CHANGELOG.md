...
### 修复：采样通道的"空表值"被当成占位，整包丢弃（通道永远发不出去）

  * **现象**（伪机床一装就现形）：`采样报文不完整，已丢弃: 通道 pseudo` 一秒一条，
    通道**从来没上报成功过**。
  * **根因**：采样编码里"某一项的值是空表"（例如"当前没有报警"的报警列表）与"本周期该项
    没有数据"的占位 `[]` **同形**；而设备端直接拿 `ncl_message_sample_is_complete()`
    当发布闸门，那一列被判成占位 → 整包丢弃 → **通道里只要有一项可能为空表，这个通道就
    永远发不出去**。文档写的本来是"占位列换成 null 之后再发"，代码少做了这一步。
  * **修复**：设备端发布前先调新公开的 `ncl_message_sample_fill_empty_columns()` 把整列 `[]`
    换成等长 null，**再**判完整性 —— 只有外层对不上（表头与列数不符、各列槽位不齐）才算
    残缺、才丢弃。两种情形因此都成立：真没采到 → 一条完整的 null 报文（"设备活着、这一刻
    没有数据"，上报节奏不受影响）；空表值 → 语义等价的 null。
    受影响的**不只伪机床**：新代（syntec）的报警列同在这条规则上 —— 没有报警时它的整个采样
    通道也在丢包。
  * **验证**：
    - 端到端（设备 + EMQX + 客户端）：修前 **9 秒 9 包全丢**；修后设备 **0 丢弃**、客户端
      8 秒收到 **8 包**（`/CONTROLLER/WARNING` 那一列按预期是 `null`）。
    - 回归用例（`stack/test/server/test_server.c` 新增一节：某项返回空表时整包照发）：
      **去掉修复就 FAIL**、装上就过。
    - 全平台矩阵重编重跑：Windows MSVC 六个变体、Linux 四套（`gcc:13` 容器内）、mingw 两套
      （PE 产物回 Windows 跑）**各 43/43**。
  * **顺带**：`build-linux.sh` 给宿主端到端测试补上 `NCL_TEST_PSEUDO_DIR`（CMake 那边早就给了）
    —— 否则非 CMake 的 Linux 构建编不出这个测试，一直静默用着旧二进制"通过"。
  * **dist**：两个包重建入库（core 186 文件 / adapter 17 文件），全平台库与示例一起换新；
    `SHA256SUMS` 独立复算 0 不符、`zip.sha256` 与字节一致。
...
### 宿主：安装根目录自动定位，配置读不到时把话说清楚
  ...
  * **现象**：拿到发布包，双击（或从 `bin\` 里敲）`bin\ncl_server.exe`，直接报
    `适配器启动失败: cannot read "…\bin\conf/device.json"` —— 安装根被当成当前目录
    （`bin\`），而 `conf\` 与 `plugins\` 在它的上一层；报错本身也只有一句 "cannot read"，
    没写清楚该拿哪份配置、该敲什么命令。
  * **安装根**（`-r/--root` 没给时）：挑**第一个像安装根**的目录 —— 当前目录、它的上一级、
    上两级里谁带 `conf/` 或 `plugins/` 就用谁；都不像才回落"当前目录"（老默认）。发布包
    `<root>\bin\ncl_server.exe` 这个形状，无论从包根跑还是从 `bin\` 里跑，根都定位到包根。
  * **`-c` 的相对路径**：先按当前目录找，读不到再按安装根目录找一次（并打一条 warn 说明
    是按哪个根读到的）—— 于是从 `bin\` 里敲 `-c conf/pseudo.json` 也指得对。
  * **报错**：读不到设备配置时，除原文外再打出**用的是哪个安装根**、`conf\` 里**现成有哪些
    `.json`**、以及一条可以直接抄的 `-c` 命令；`conf\` 里一份都没有时写明"配置要站点自己写
    （格式见随包 README 第 2 节）"。`--help` 里的 `-r` 说明同步改写。
  * **验证**：`--plugins` / `--once` 分别从**包根**与**包里的 `bin\`** 各跑一遍（都定位到
    包根、自检 72/72）；构造"像发布包但 `conf\` 里没有配置"与"`conf\` 里有三份样例"两个目录，
    两条报错路径都按预期打出清单与现成命令；`ctest` **43/43**。
  * 顺带把包内运行期产物写进 `.gitignore`（`dist/**/bin/sn.txt`、`dist/**/conf/device.json`、
    `dist/**/log/`）—— 它们不在包自己的 SHA256SUMS 里，试跑一次就会冒出来，别跟着 dist 入库。
